...
public class Main {
    ...
    public static void main( String[] args) {
        ...
        Bus Bus = new Bus("Black", "17", 3590, "1955", CarBrand.BMW, 17, 23);
        Car Car = new Car("Red", "19", 8354, "1999", VolumeMotor.Big, 17, 2.0);
        Truck Truck = new Truck("White", "21", 19572, "2010",  Motor.DIESEL, 15, 27);
        ...
        Auto resultGraduationYear  =  List.stream().filter(x -> "1955".equals(
                x.getGraduationYear())).findAny().orElse(null);
        System.out.println("\n" + resultGraduationYear);

        manager.findWheelDiameter(List, "19");
        System.out.println("WheelDiameter: ");
        Auto resultWheelDiameter  =  List.stream().filter(x -> "19".equals(
                x.getWheelDiameter())).findAny().orElse(null);
        System.out.println("\n" + resultWheelDiameter);
    }
}
